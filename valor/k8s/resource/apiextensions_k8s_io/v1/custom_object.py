...
class CustomObject(K8sResource):
    ...
    def get_from_cluster(
        ...
        if "group" not in kwargs:
            logger.error("No Group provided when reading CustomObject")
            return None
        if "version" not in kwargs:
            logger.error("No Version provided when reading CustomObject")
            return None
        if "plural" not in kwargs:
            logger.error("No Plural provided when reading CustomObject")
            return None
        ...
        try:
            ...
            logger.warning(f"Could not read custom objects for: {group}/{version}: {e}")
