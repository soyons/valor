...
from valor.api.workspace import log_workspace_event
from valor.api.schemas.workspace import (
    WorkspaceSchema,
    WorkspaceCreate,
    WorkspaceUpdate,
    WorkspaceEvent,
...
from valor.cli.config import PhiCliConfig
from valor.cli.console import (
    console,
    print_heading,
    print_info,
    print_subheading,
...
from valor.infra.type import InfraType
from valor.infra.resources import InfraResources
from valor.workspace.config import WorkspaceConfig
from valor.workspace.enums import WorkspaceStarterTemplate
...
def create_workspace(name: Optional[str] = None, template: Optional[str] = None, url: Optional[str] = None) -> bool:
    ...
    from valor.cli.operator import initialize_phi
    from valor.utils.common import str_to_int
    from valor.utils.filesystem import rmdir_recursive
    from valor.workspace.helpers import get_workspace_dir_path
    from valor.utils.git import GitCloneProgress
    ...
    if not phi_config:
        ...
        if not init_success:
            from valor.cli.console import log_phi_init_failed_msg
...
def setup_workspace(ws_root_path: Path) -> bool:
    ...
    """
    from valor.cli.operator import initialize_phi
    from valor.utils.git import get_remote_origin_for_dir
    from valor.workspace.helpers import get_workspace_dir_path
    ...
    if phi_config.user is None:
        from valor.api.user import create_anon_user
    ...
    if phi_config.user is not None:
        ...
        if ws_schema is None or ws_schema.id_workspace is None:
            from valor.api.workspace import create_workspace_for_user
            from valor.workspace.helpers import generate_workspace_name
        ...
        ######################################################
            ...
            from valor.api.workspace import update_primary_workspace_for_user
        ...
        if ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from valor.api.workspace import update_workspace_for_user
